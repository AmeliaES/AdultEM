...
write.csv(perm, file = 'test.csv')
  ...
  index2 = match(6143855, complete_MVP2$d$PointNo)
  neuron.distal2 = distal_to(complete_MVP2, index2)
  neuron.distal.points2 = complete_MVP2$d[neuron.distal2,]
  points3d(neuron.distal.points2[,c('X','Y','Z')], col = "cyan")

  #colour branch of neuron distal to point a different colour

  outgoing_connections2 <- function(skid, cut_point) {
    #get neuron of interest (MVP2) and find connectors in arbour of interest (in pedc)
    neuron = read.neuron.catmaid(skid)
    index = match(cut_point, neuron$d$PointNo)
    neuron.distal = distal_to(neuron, index)
    neuron.distal.points = neuron$d[neuron.distal,]
    points3d(neuron.distal.points[,c('X','Y','Z')], col = "cadetblue")
  }
  
    outgoing_connections3 <- function(skid, cut_point) {
    #get neuron of interest (MVP2) and find connectors in arbour of interest (in pedc)
    neuron = read.neuron.catmaid(skid)
    index = match(cut_point, neuron$d$PointNo)
    neuron.distal = distal_to(neuron, index)
    neuron.distal.points = neuron$d[neuron.distal,]
    points3d(neuron.distal.points[,c('X','Y','Z')], col = "darkolivegreen4")
  }
outgoing_connections(2333007, 6143288)
outgoing_connections2(2333007, 6143855)
outgoing_connections3(2333007, 11583672)
